...
window.onload = ()=>{
    ...
    get("/getWebSocketUrl")
        .then((data) => {
            ...
            websocket.onopen = () => {
                ...
                // websocket.send(JSON.stringify({command:TYPE_COMMAND_IMG})); //发送图片
            };
            websocket.onclose = () => {
    ...
    get("/getWebSocketUrlFile")
        .then((data) => {
            if (!websocketFile) {
                websocketFile = new WebSocket(data.url);
                log("websocket连接成功File服务器")
            }
            websocketFile.onopen = () => {
                // websocket.send(JSON.stringify({command:TYPE_COMMAND_ROOM_LIST}))
                // websocket.send(JSON.stringify({command:TYPE_COMMAND_IMG})); //发送图片
            };
            websocketFile.onclose = () => {
                log("Connection closed.");
            };
            websocketFile.onerror = () => {
                log("websocket error");
            };
            websocketFile.onmessage = function(event) {
                var aaa = document.getElementById('file');
                var html;
                console.log(aaa)
                html=document.createElement('a');
                html.setAttribute('href', "/file/"+event.data);
                html.style.display='block';
                html.target='_blank'
                html.innerText=event.data
...
const handleMessage = (event) => {
    console.log(event);
    log(event.data);
    // alert(typeof(event.data) =="string")
...
function setImagePreview1() {
    ...
    websocketFile.send(fileName);
    var reader = new FileReader();
    reader.readAsArrayBuffer(file);
    reader.onload = function(e) {
        var rawData = e.target.result;
        websocketFile.send(rawData);
    };
}
